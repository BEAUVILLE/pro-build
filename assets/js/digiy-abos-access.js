/* ============================================================
   DIGIYLYFE · MES SERVICES · ABOS ACCESS BRIDGE
   À poser dans : assets/js/digiy-abos-access.js

   Nom technique interne : BUILD
   ...
   Important :
   - BUILD est encore à cadrer dans le rail.
   - Ne pas bloquer MES SERVICES avec protect() tant que le plan n’est pas actif.
   ============================================================ */
...
(function () {
    ...
    return cleanPhone(
      readQuery("phone") ||
        ...
        readStorage([
          ...
          "DIGIY_SESSION_PHONE",
          "DIGIY_BUILD_PHONE",
          "DIGIY_BUILD_SESSION_PHONE",
          "DIGIY_BUILD_LAST_PHONE",
          "DIGIY_SERVICES_PHONE",
          "DIGIY_SERVICES_LAST_PHONE",
          "DIGIY_DRIVER_PHONE",
          "DIGIY_LOC_PHONE",
          "DIGIY_MARKET_PHONE",
          "DIGIY_PAY_PHONE"
  ...
  function guessModule(options) {
    ...
    if (module === "SERVICES") return "BUILD";
    if (module === "MES_SERVICES") return "BUILD";
    return module;
  }
  ...
  function savePhone(phone, module) {
    ...
    try {
      ...
      if (m === "BUILD") {
        localStorage.setItem("DIGIY_BUILD_PHONE", p);
        localStorage.setItem("DIGIY_BUILD_LAST_PHONE", p);
        localStorage.setItem("DIGIY_SERVICES_PHONE", p);
        localStorage.setItem("DIGIY_SERVICES_LAST_PHONE", p);
      }
    } catch (_) {}
  }
  ...
  function renderAccessBadge(target, result) {
    ...
    if (result.has_access) {
      el.innerHTML = `
        <strong>✅ Accès MES SERVICES actif</strong><br>
        ${result.fiche_title || "MES SERVICES · DIGIY"}<br>
