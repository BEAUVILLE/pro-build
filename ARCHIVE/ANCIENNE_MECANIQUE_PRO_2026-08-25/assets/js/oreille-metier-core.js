/* DIGIYLYFE — OREILLE MÉTIER CORE · MES SERVICES
...
(function(){
  ...
  var VERSION = "oreille-metier-core-services-paves-tel-20260524";
  ...
  function storeKey(cfg){
    return String((cfg && cfg.storagePrefix) || "DIGIY_OREILLE_METIER") +
      "_" +
      String((cfg && cfg.module) || "BUILD").toUpperCase().replace(/[^A-Z0-9]+/g, "_") +
      "_NOTES_V1";
  }
  ...
  function saveNote(cfg, text, extra){
    ...
    var note = assign({
      id:"note_" + Date.now(),
      module:String((cfg && cfg.module) || "BUILD").toUpperCase(),
      text:clean,
      date:now(),
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .digiy-oreille-status{border-radius:18px!important;background:#052e16!important;color:#d8ffe8!important;padding:13px 14px!important;font-size:1.05rem!important;font-weight:1000!important;line-height:1.34!important;margin:10px 0!important;border:1px solid rgba(250,204,21,.24)!important}
  ...
  function renderNotes(box, cfg){
    ...
    box.innerHTML = "";
    if(!list.length){
      box.innerHTML = "<div class=\"digiy-oreille-note\"><b>Aucune note rangée</b><span>Teste une suggestion, puis clique sur Ranger.</span></div>";
      return;
    }
    list.forEach(function(note){
      var div = document.createElement("div");
      div.className = "digiy-oreille-note";
      div.innerHTML = "<b>" + esc(note.module || cfg.module || "BUILD") + "</b><div>" + esc(note.text) + "</div><small>" + esc(note.date || "") + "</small>";
      box.appendChild(div);
    });
  }
  ...
  function defaultFormulate(text, cfg){
    var clean = norm(text);
    var moduleName = String((cfg && cfg.module) || "BUILD").toUpperCase();
    return clean ? moduleName + " · Note métier : " + clean + " À vérifier, modifier et valider par le pro avant envoi ou rangement." : moduleName + " · Note vide : préciser la demande avant validation.";
  }
  ...
  function mount(userConfig){
    ...
    target.addEventListener("click", function(event){
      ...
      if(action === "save"){
        ...
        if(saved){ refreshNotes(); setStatus("Note rangée localement. Le terrain garde la main."); toast("Note rangée"); }
    ...
    refreshNotes();
    return {config:cfg,target:target,refreshNotes:refreshNotes,getText:function(){ return textArea.value; },setText:function(value){ textArea.value = norm(value); }};
  }
